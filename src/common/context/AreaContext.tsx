...
export const AreaProvider = ({children}: AreaListContextProps) => {
    ...
    function _handleSave({info, data, list}) {
        ...
        .then(resp => resp.ok ? setAreaList(list) : console.log(resp.json()))
    ...
    function changeViewOrder (area: string, position: string) {
        ...
        newAreaList.splice((Number(position)-1), 0, newAreaList.splice(beforePosition, 1)[0])

        _handleSave({info: 'viewOrder', data: newAreaList, list: newAreaList})
    }
    
    return (
        <AreaContext.Provider value={{areaList, addArea, deleteArea, changeViewOrder}}>
            {children}
